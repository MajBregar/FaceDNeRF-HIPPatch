...
def run(
    ...
    id_loss = IDLoss()
    guidance = StableDiffusion(torch.device('cuda'), True, False, '1.5', None, [0.02, 0.98])
    relight_model = relight_model.to(torch.device('cuda'))


    safe_name = f"{image_name}_{description}_{lamda_id}_{lamda_origin}_{lamda_diffusion}_{lamda_illumination}".replace(' ', '_')
    ...
    guidance = StableDiffusion(torch.device('cuda:0'), True, False, '1.5', None, [0.02, 0.98])

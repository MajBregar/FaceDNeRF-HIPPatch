 ...
 *
 * Adapted for HIP/ROCm by <your name>.
 */
...
struct filtered_lrelu_kernel_spec
{
    ...
    int     numWarps;           // Number of warps per thread block.
    int     xrep;               // For processing multiple horizontal tiles per thread block.
    int     dynamicSharedKB;    // Dynamic shared memory size requested.
};
...
filtered_lrelu_kernel_spec choose_filtered_lrelu_kernel(
    const filtered_lrelu_kernel_params& p,
    int sharedKB
);

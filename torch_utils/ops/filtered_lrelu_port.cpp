    ...
    float gain, float slope, float clamp, bool flip_filters, bool writeSigns)
{
    ...
    HIP_CHECK(hipDeviceGetAttribute(&maxSharedBytes, hipDeviceAttributeMaxSharedMemoryPerBlock, x.device().index()));
    ...
    // Filter setup kernel
    HIP_CHECK(hipLaunchKernel(spec.setup, dim3(1), dim3(1024), args, 0, at::hip::getCurrentHIPStream()));

    if      ( writeSigns && !readSigns) HIP_CHECK((copy_filters<true,  false>(at::hip::getCurrentHIPStream())));
    else if (!writeSigns &&  readSigns) HIP_CHECK((copy_filters<false, true >(at::hip::getCurrentHIPStream())));
    else if (!writeSigns && !readSigns) HIP_CHECK((copy_filters<false, false>(at::hip::getCurrentHIPStream())));

    // Shared mem setup
    HIP_CHECK(hipFuncSetCacheConfig(spec.exec, hipFuncCachePreferShared));
    if (spec.dynamicSharedKB)
        HIP_CHECK(hipFuncSetAttribute(spec.exec, hipFuncAttributeMaxDynamicSharedMemorySize, spec.dynamicSharedKB << 10));
    ...
    for (int zofs = 0; zofs < gz; zofs += maxSubGz)
    {
        ...
        HIP_CHECK(hipLaunchKernel(spec.exec, dim3(gx, gy, subGz), dim3(bx), args, spec.dynamicSharedKB << 10, at::hip::getCurrentHIPStream()));
    }
    ...
    float gain, float slope, float clamp, bool writeSigns)
{
    ...
    // Launch.
    HIP_CHECK(hipLaunchKernel(func, dim3(gx, gy, gz), dim3(bx), args, 0, at::hip::getCurrentHIPStream()));

...
def project_pti(
    ...
    if target_images_orginal_illu.shape[2] > 256:
        target_images = F.interpolate(target_images_orginal_illu, size=(256, 256), mode='area')
    target_features = vgg16(target_images, resize_images=False, return_lpips=True).to(torch.device('cuda:0'))
    vgg16 = vgg16.to(torch.device('cuda:0'))
    id_loss = id_loss.to(torch.device('cuda:0'))
    target_images = target_images.to(torch.device('cuda:0'))
    torch.cuda.empty_cache()
    # target_id_features = 
    ...
    for step in tqdm(range(num_steps_pti)):
        ...
        # w_noise = torch.randn_like(w_opt) * w_noise_scale
        # ws = (w_opt + w_noise)
        synth_images = G.synthesis(w_pivot,c, noise_mode='const')['image'].to(torch.device('cuda:0'))
        #synth_images size: [1, 3, 512, 512]
        ...
        # w_pivot = w_pivot.to(torch.device('cuda:0'))
        side_synth_images = G.synthesis(w_pivot,side_c, noise_mode='const')['image'].to(torch.device('cuda:0'))
        side_synth_images_print = (side_synth_images + 1) * (255 / 2)  # original size is 512
        side_synth_images = (side_synth_images + 1) /2.0
        side_synth_images = F.interpolate(side_synth_images, size=(512, 512), mode='area')
        # ################print out the sides views
        ...
        #################
        #diffusion loss
        guidance = guidance.to(torch.device('cuda:0'))
        text_inputs = guidance.get_text_embeds([text_prompt], [""])
        d_loss,real_d_loss = guidance.train_step(text_inputs, side_synth_images, as_latent=False)
        logging.info(str(step)+" d_loss: "+str(f'{real_d_loss.cpu().detach():.20f}'))
        dist = dist.to(torch.device('cuda:0')) + d_loss[0] * weight_of_d_loss
          
        ######################
        # illu loss
        sh = SH_project_polar_function(getLightIntensity,phi,theta)
        sh = np.array(sh) * 2.5
        ill_loss = illu_loss(relight_model,sh,side_synth_images,torch.device('cuda:0')) #input sh (type numpy)
        logging.info(str(step)+" illu loss: "+str(f'{ill_loss.cpu().detach():.20f}'))
        ...
        reg_loss = 0.0
        for v in noise_bufs.values():
            noise = v[None, None, :, :].to(torch.device('cuda:0'))  # must be [1,1,H,W] for F.avg_pool2d()
            while True:
                reg_loss += (noise * torch.roll(noise, shifts=1, dims=3)).mean() ** 2
                reg_loss += (noise * torch.roll(noise, shifts=1, dims=2)).mean() ** 2
                if noise.shape[2] <= 8:
